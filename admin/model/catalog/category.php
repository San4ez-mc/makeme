...
class ModelCatalogCategory extends Model
{
    public function addCategory($data)
    {
        ...
        if (isset($data['image'])) {
            $this->db->query("UPDATE " . DB_PREFIX . "category SET hover_image = '" . $this->db->escape($data['hover_image']) . "' WHERE category_id = '" . (int)$category_id . "'");
        }
        ...
        if (isset($data['boxing_id'])) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "boxing_to_category SET category_id = '" . (int)$category_id . "', boxing_id = '" . (int)$data['boxing_id'] . "'");
        }
    ...
    public function editCategory($category_id, $data)
    {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "boxing_to_category WHERE category_id = '" . (int)$category_id . "'");
    ...
    public function getCategoryBoxing($category_id)
    {
        $category_boxing_data = array();

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "boxing_to_category btc
         LEFT JOIN " . DB_PREFIX . "boxing b ON b.boxing_id = btc.boxing_id
         WHERE category_id = '" . (int)$category_id . "'");

        foreach ($query->rows as $result) {
            $category_boxing_data[] = ['id' => $result['boxing_id'], 'name' => $result['name']];
        }

        return count($category_boxing_data) > 0 ? $category_boxing_data[0] : ['id' => null, 'name' => null];
    }
