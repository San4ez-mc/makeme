...
//        var_dump($this->request->post['product_components']);die();
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
    ...
    protected function getForm()
    {
        ...
        if (isset($this->request->post['product_components'])) {
            $components = $this->request->post['product_components'];
        } elseif (isset($this->request->get['product_id'])) {
            $components = $this->model_catalog_product->getProductComponents($this->request->get['product_id']);
        } else {
            $components = array();
        }

        $data['product_components'] = array();

        foreach ($components as $component_id) {
            $component_info = $this->model_catalog_component->getComponent($component_id);
