...
class ControllerBlogArticle extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($this->request->get['blog_category_id'])) {
            ...
            if (!empty($cats)) {
                $blog_category_id = $cats[0]['blog_category_id'];
        ...
        if ($article_info) {
            ...
            $data['date_added'] = date($this->language->get('date_format_short2'), strtotime($article_info['date_added']));
            ...
            foreach ($results as $result) {
                ...
                $cats = $this->model_blog_article->getCategories($result['article_id']);

                if (!empty($cats)) {
                    $blog_category_id_ = $cats[0]['blog_category_id'];
                    $related_category_info = $this->model_blog_category->getCategory($blog_category_id_);
                }

                $date_formatted = date($this->language->get('date_format_day'), strtotime($result['date_added'])) .
                    $this->language->get(date($this->language->get('date_format_month'), strtotime($result['date_added']))) .
                    date($this->language->get('date_format_year'), strtotime($result['date_added']));

                $data['articles'][] = array(
                    'article_id' => $result['article_id'],
                    'thumb' => $image,
                    'name' => $result['name'],
                    'category_name' => !empty($related_category_info) ? $related_category_info['name'] : '',
                    'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('configblog_article_description_length')) . '..',
                    'rating' => $rating,
//                    'date_added' => date($this->language->get('date_format_text'), strtotime($result['date_added'])),
            ...
            $constructor_article = $this->model_blog_article->getArticle(130);

            if ($constructor_article['image']) {
                $image = $this->model_tool_image->resize($constructor_article['image'], $this->config->get('configblog_image_related_width'), $this->config->get('configblog_image_related_height'));
            } else {
                $image = $this->model_tool_image->resize('placeholder.png', $this->config->get('configblog_image_related_width'), $this->config->get('configblog_image_related_height'));
            }

            $data['constructor_article'] = array(
                'article_id' => $constructor_article['article_id'],
                'thumb' => $image,
                'name' => $constructor_article['name'],
                'href' => $this->url->link('blog/article', 'article_id=' . $constructor_article['article_id']),
            );

            $data['likes'] = $this->model_blog_article->getLikes($article_id);
            $data['liked'] = $this->model_blog_article->isLiked($article_id, !empty($this->session->data['user_id']) ? $this->session->data['user_id'] : null);
            if (empty($this->session->data['user_id']) && $data['liked']) {
    ...
    public function like()
    {
        if (!empty($this->request->post['article_id'])) {
            ...
            $liked = $this->request->post['liked'];
            $user_id = !empty($this->session->data['user_id']) ? $this->session->data['user_id'] : null;
            $this->load->language('blog/article');
            $this->load->model('blog/article');
            try {
                if (!empty($user_id)) {
                    if ($liked === "true") {
                        $this->model_blog_article->addLike($article_id, $user_id);
                    } else {
                        $this->model_blog_article->removeLike($article_id, $user_id);
                    }
                    $json = [
                        'status' => 'success',
                        'likes' => $this->model_blog_article->getLikes($article_id),
                        'message' => $this->language->get('text_like_success')
                    ];
                } else {
                    $likes = $this->model_blog_article->getLikes($article_id);
                    if ($liked === "true") {
                        $this->session->data['liked'][] = $article_id;
                        $json = [
                            'status' => 'success',
                            'likes' => $likes + 1,
                            'message' => $this->language->get('text_like_success')
                        ...
                        if (!empty($this->session->data['liked'])) {
                            foreach ($this->session->data['liked'] as $key => $liked) {
                                if ($liked === $article_id) {
