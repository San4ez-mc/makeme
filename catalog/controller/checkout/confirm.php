...
class ControllerCheckoutConfirm extends Controller
{
    public function index()
    {
        ...
        $data['name'] = $this->session->data['payment_address']['firstname'];
        $data['surname'] = $this->session->data['payment_address']['lastname'];
        $data['phone'] = $order_data['telephone'];
        $data['email'] = $order_data['email'];
        $data['address'] = $this->session->data['payment_address']['address_1'];
        $data['paymentMethod'] = $this->session->data['payment_method']['name'];
...
//        $this->cart->clear();
    ...
    public function ajax()
    {
        if ($this->request->server['REQUEST_METHOD'] == 'POST' &&
            ...
            !empty($this->request->post['surname']) &&
            !empty($this->request->post['email']) &&
            !empty($this->request->post['phone'])) {
...
//                    $city = $this->model_localisation_city->getCityByZoneId($this->request->post['city_id']);
                    ...
                    $this->session->data['shipping_method'] = [
                        'title' => 'Доставка в отделение Новой Почты',
                        'cost' => 0,
                        'tax_class_id' => 0,
                    ];
                    break;
            ...
            switch ($this->request->post['paymentMethod']) {
                case 'free_checkout':
                    $this->session->data['payment_method'] = [
                        'code' => $this->request->post['paymentMethod'],
            ...
            $comment = '';
            if ($this->request->post['no_call'] == 1) {
                $comment = 'Мне можно не перезванивать<br/>';
            }
