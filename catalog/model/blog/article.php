...
class ModelBlogArticle extends Model
{
    ...
    public function getLikes($article_id, $user_id = null)
    {
        $where = "WHERE article_id = " . $article_id;
        if ($user_id) {
            $where .= " AND user_id = " . $user_id;
        }
        $likes = $this->db->query("SELECT COUNT(user_id) AS likes FROM " . DB_PREFIX . "article_likes_to_users " . $where);

        return !empty($likes) ? $likes->row['likes'] : 0;
    }

    public function addLike($article_id, $user_id)
    {
        if (!empty($article_id) && !empty($user_id)) {
            $is_exist = $this->getLikes($article_id, $user_id);
            if (empty($is_exist)) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "article_likes_to_users SET article_id = '" . (int)$article_id . "', user_id = '" . (int)$user_id . "'");
            }
        }
    }
    ...
    {
        if (!empty($article_id) && !empty($user_id)) {
            $this->db->query("DELETE FROM " . DB_PREFIX . "article_likes_to_users WHERE article_id = '" . (int)$article_id . "'AND user_id = '" . (int)$user_id . "'");
        }
    }

    public function isLiked($article_id, $user_id)
    {
        if (!empty($article_id)) {
            if (!empty($user_id)) {
                return !empty($this->getLikes($article_id, $user_id));
            } else {
