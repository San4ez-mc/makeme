...
class ModelCatalogFilter extends Model
{
    ...
    public function getFilterGroups($data = array())
    {
        ...
        if (isset($data['filter_group_id'])) {
            $sql .= " AND fg.filter_group_id = " . $data['filter_group_id'];
        }
    ...
    public function getFilterGroupsWithFilters($data = array())
    {
        $filterGroups = $this->getFilterGroups($data);
        if (!empty($filterGroups)) {
            $filters = $this->getFilters([]);
            if (!empty($filters)) {
                foreach ($filterGroups as &$filterGroup) {
                    foreach ($filters as $filter) {
                        if ($filterGroup['filter_group_id'] === $filter['filter_group_id']) {
