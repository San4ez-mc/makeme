...
class ModelCatalogProduct extends Model
{
    ...
    public function getProducts($data = array())
    {
        ...
        if (!empty($data['filter_categories']) || !empty($data['filter_categories_off'])) {
        ...
        if (!empty($data['filter_filters_off'])) {
            $sql .= " AND pf.filter_id NOT IN (" . $data['filter_filters_off'] . ")";
        }
        ...
        if (!empty($data['filter_categories'])) {
            $sql .= " AND p2c.category_id IN ( " . $data['filter_categories'] . ")";
        }
        if (!empty($data['filter_categories_off'])) {
            $sql .= " AND p2c.category_id NOT IN ( " . $data['filter_categories_off'] . ")";
        }
    ...
    public function getProductsEndPrices()
    {
        $query = $this->db->query("SELECT MAX(price) as max_price, MIN(price) as min_price FROM `" . DB_PREFIX . "product`");
    ...
    public function editProductPublic()
    {
        $this->db->query("UPDATE " . DB_PREFIX . "product SET available_to_all = '" . $this->db->escape($public) . "' WHERE product_id = '" . (int)$product_id . "'");
    }
}
